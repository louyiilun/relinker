...
def find_last_checkpoint(checkpoints_dir):
    ...
    return latest_fname, os.path.join(checkpoints_dir, latest_fname)

def refresh_config_file(config_file, latest_fname):
    max_epoches = int(latest_fname.split('=')[1].split('.')[0])
    current_epoch = max_epoches + 10
    print(f'========Current number of epochs is {current_epoch}========')
    with open(config_file, 'r') as file:
        yaml_text = file.read()
    yaml_text = re.sub(r'n_epochs:\s*\d+', f'n_epochs: {current_epoch}', yaml_text)
...
def main(args):
    ...
    # experiment = args.checkpoints
    # checkpoints_dir = os.path.join(experiment)
    ...
    wandb_logger = loggers.WandbLogger(
        ...
        id=experiment,
        # resume='must',
        resume='must' if resume is not None else 'allow',
    ...
    # else:
    #     latest_fname, last_checkpoint = find_last_checkpoint(checkpoints_dir)
    # print(f'========Training will be resumed from the latest checkpoint {last_checkpoint}========')

    # refresh_config_file(args.config, latest_fname)
...
if __name__ == '__main__':
    ...
    p.add_argument('--scoring_function', action='store', type=str, required=True) # added scoring function type
    p.add_argument('--samples_path', action='store', type=str,  default="samples")
    ...
    reinforce_file_path = os.path.join(args.samples_path, f'{args.scoring_function}_value.txt')
    try:
        with open(reinforce_file_path, "r") as file:
            reinforce = file.readline().strip()
        rl_value = float(reinforce)
    except ValueError:
        print('========== the reinforce value nowhere to find ==========')

    # if args.resume is None:
    #     last_checkpoint = None
    #     print(f'========Training will not be resumed========')
    #     exit()
    # else:
    #     checkpoints_dir = os.path.join(args.checkpoints, args.resume)
    #     latest_fname, last_checkpoint = find_last_checkpoint(checkpoints_dir)
    #     print(f'========Training will be resumed from the latest checkpoint {last_checkpoint}========')
    #     refresh_config_file(args.config, latest_fname)
    ...
    if os.listdir(model_path) == None:
        ...
        resume = os.listdir(model_path)[0]
        checkpoints_dir = os.path.join(args.checkpoints, resume)
        latest_fname, last_checkpoint = find_last_checkpoint(checkpoints_dir)
        print(f'========Training will be resumed from the latest checkpoint {last_checkpoint}========')
        refresh_config_file(args.config, latest_fname)
