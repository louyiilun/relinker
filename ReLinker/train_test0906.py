...
def cal_logp(samples_path):
    logplist = []
    smilelist = []
    pred_mols = []
    goal_ClogP =2.5
    obabel_dir = f'{samples_path}/obabel'
    for fname in os.listdir(samples_path):
        number = fname.split('_')[0]
        if number.isdigit():
            pred_obabel_path = f'{obabel_dir}/{number}_.sdf'
        supp = Chem.SDMolSupplier(pred_obabel_path, sanitize=False)
        mol = list(supp)[0]
        ...
        pred_mols.append(mol_filtered)

    for m in pred_mols:
        try:
            mol = Chem.AddHs(m)
            logp = Chem.MolLogP(mol)
            logplist.append(logp)
        except:
            exception_smi = Chem.MolToSmiles(m)
            smilelist.append(exception_smi)
...
def main(args):
    ...
    rclogp = cal_logp(args.samples_path)
