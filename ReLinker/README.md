...
Run trainig:
```shell
python -W ignore train_test.py --config configs/zinc_difflinker_test.yml
```
...
<!-- addition: -->
Firstly, set the reinforce scoring function as the $reinforce and then continue:
```shell
mkdir -p $reinforce/model/difflinker_test_reinforce_$reinforce
cd $reinforce/model/difflinker_test_reinforce_$reinforce
wget https://zenodo.org/record/7121300/files/zinc_difflinker.ckpt?download=1 -O zinc_difflinker.ckpt
mv zinc_difflinker.ckpt $reinforce/model/difflinker_test_reinforce_"$reinforce"_epoch=00.ckpt



wget https://zenodo.org/record/7121300/files/zinc_difflinker.ckpt?download=1 -O sa/model/difflinker_test_reinforce_sa/zinc_difflinker.ckpt

mv zinc_difflinker.ckpt difflinker_test_reinforce_sa_epoch=00.ckpt
```

delete args.resume
add a resume variable
set resume=none as default
if path/model is empty, last checkpoint is none
if path/model is not empty, last checkpoint is os.listdir(model) and choose the first directory

CUDA_VISIBLE_DEVICES=4 nohup bash run.sh > output_sa1114.log 2>&1 &


CUDA_VISIBLE_DEVICES=4 nohup bash run.sh > output_qed1114.log 2>&1 &
